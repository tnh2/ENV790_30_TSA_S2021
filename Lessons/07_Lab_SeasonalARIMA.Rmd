...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # Separator is space (" "), original column is space-[Month]-space-Average
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
...
ggplot(BR_complete) +
  geom_line(aes(x = Date, y = Rainfall)) +
  geom_smooth(aes(x = Date, y = Rainfall), method = "lm") +
  labs(title = "Rainfall Over Time", y = "Rainfall (mm)")
...
Rainfall_ts <- ts(BR_complete$Rainfall, start = c(year(BR_complete$Date[1]), month(BR_complete$Date)), frequency = 12)
Temp_ts <- ts(BR_complete$Temperature_C, start = c(year(BR_complete$Date[1]), month(BR_complete$Date)), frequency = 12)
...
Rainfall_decomp <- stl(Rainfall_ts, s.window = "period")
Temp_decomp <- stl(Temp_ts, s.window = "period")
...
temp_deseas <- seasadj(Temp_decomp)
temp_d <- nsdiffs(Temp_decomp)
temp_D <- ndiffs(temp_deseas)
adf.test(Temp_ts)

temp_diff <- diff(Temp_ts, differences = temp_d, lag = 1)
