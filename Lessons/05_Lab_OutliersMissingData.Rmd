...
clean_wind_data <- tsclean(ts_wind_data)  #object should be a time series - can be used with seasonal or trend data
...
1) You have two options: alternative (additive?) and multiplicative. Multiplicative models exhibit a change in frequency over time.
...
<<<<<<< HEAD
window <-  6
mavg <- array(NA, length(clean_wind_data-12))

for (i in 7:length(clean_wind_data)) {
  #mavg[i] <- mean(clean_wind_data[c((i-window):(i-1), (i+1):(i+window))])
  mavg[i] <- mean(clean_wind_data[(i-window):(i+window)])
}

ggplot() +
  geom_line(aes(x =c(1:length(mavg)), y=ma(clean_wind_data, 12))) #+
  geom_line(aes(x = c(1:length(mavg)), y = wind_trend), color = "red")
